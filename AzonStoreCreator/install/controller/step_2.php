...
class ControllerStep2 extends Controller {
	...
	private function validate() {
		if (phpversion() < '5.3') {
			$this->error['warning'] = 'Warning: You need to use PHP5.3 or above for Azon Store to work!';
		}
		...
		if (ini_get('session.auto_start')) {
			$this->error['warning'] = 'Warning: Azon Store will not work with session.auto_start enabled!';
		}

		if (!array_filter(array('mysql', 'mysqli', 'pdo', 'pgsql'), 'extension_loaded')) {
			$this->error['warning'] = 'Warning: A database extension needs to be loaded in the php.ini for Azon Store to work!';
		}

		if (!extension_loaded('gd')) {
			$this->error['warning'] = 'Warning: GD extension needs to be loaded for Azon Store to work!';
		}

		if (!extension_loaded('curl')) {
			$this->error['warning'] = 'Warning: CURL extension needs to be loaded for Azon Store to work!';
		}

		if (!function_exists('mcrypt_encrypt')) {
			$this->error['warning'] = 'Warning: mCrypt extension needs to be loaded for Azon Store to work!';
		}

		if (!extension_loaded('zlib')) {
			$this->error['warning'] = 'Warning: ZLIB extension needs to be loaded for Azon Store to work!';
		}

		if (!extension_loaded('zip')) {
			$this->error['warning'] = 'Warning: ZIP extension needs to be loaded for Azon Store to work!';
		}

		if (!function_exists('iconv')) {
			if (!extension_loaded('mbstring')) {
				$this->error['warning'] = 'Warning: mbstring extension needs to be loaded for Azon Store to work!';
			}
		}
		...
		if (!file_exists(DIR_OPENCART . 'config.php')) {
			...
			$this->error['warning'] = 'Warning: config.php needs to be writable for Azon Store to be installed!';
		}
		...
		if (!file_exists(DIR_OPENCART . 'admin/config.php')) {
			...
			$this->error['warning'] = 'Warning: admin/config.php needs to be writable for Azon Store to be installed!';
		}

		if (!is_writable(DIR_SYSTEM . 'cache')) {
			$this->error['warning'] = 'Warning: Cache directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_SYSTEM . 'logs')) {
			$this->error['warning'] = 'Warning: Logs directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_SYSTEM . 'download')) {
			$this->error['warning'] = 'Warning: Download directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_SYSTEM . 'upload')) {
			$this->error['warning'] = 'Warning: Upload directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_OPENCART . 'image')) {
			$this->error['warning'] = 'Warning: Image directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_OPENCART . 'image/cache')) {
			$this->error['warning'] = 'Warning: Image cache directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_OPENCART . 'image/catalog')) {
			$this->error['warning'] = 'Warning: Image catalog directory needs to be writable for Azon Store to work!';
		}

		if (!is_writable(DIR_SYSTEM . 'modification')) {
			$this->error['warning'] = 'Warning: Modification directory needs to be writable for Azon Store to work!';
		}
